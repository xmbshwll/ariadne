	...
	tests := []struct {
	...
	}{
		{
			...
			wantStdout: []string{
				...
				"--verbose                         print full resolution details (values: true or false; false emits compact links, true emits metadata, scores, reasons, and alternates)",
				"--format string                   output format (values: json for structured output, yaml for YAML, csv for spreadsheet-friendly export)",
				"--services string                 comma-separated target services (values: appleMusic, bandcamp, deezer, soundcloud, spotify, tidal, youtubeMusic, ytmusic; ytmusic aliases youtubeMusic)",
				"--min-strength string             minimum match strength (values: very_weak, weak, probable, strong; filters weaker results out of the final output)",
				"--apple-music-storefront string   preferred Apple Music storefront (values: ISO 3166-1 alpha-2 code such as us, gb, de, fr, jp, ca, au; used when the source URL has no storefront)",
				"The CLI now loads configuration through Viper",
