...
import (
	...
	"github.com/xmbshwll/ariadne/internal/adapters/soundcloud"
...
func TestAdapterParseMethodsDelegateToPackageParses(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			raw:            "https://soundcloud.com/artist/sets/set-name",
			wantEntityType: model.EntityTypeAlbum,
		},
		{
			name:        "a non-album url is refused with the service sentinel text",
			raw:         "https://soundcloud.com/artist/likes",
			wantErrText: "soundcloud url is not an album-like set url",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			funcParsed, funcErr := soundcloud.ParseAlbumURL(tt.raw)
			...
			if methodParsed != nil {
				assert.Equal(t, model.ServiceSoundCloud, methodParsed.Service, tt.name)
