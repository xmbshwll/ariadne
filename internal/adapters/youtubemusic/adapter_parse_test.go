...
import (
	...
	"github.com/xmbshwll/ariadne/internal/adapters/youtubemusic"
...
func TestAdapterParseMethodsDelegateToPackageParses(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			raw:         "https://music.youtube.com/channel/xyz",
			wantErrText: "youtube music url is not an album url",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			funcParsed, funcErr := youtubemusic.ParseAlbumURL(tt.raw)
			...
			if methodParsed != nil {
				assert.Equal(t, model.ServiceYouTubeMusic, methodParsed.Service, tt.name)
